...
impl Kubernetes {
    fn is_kubeconfig_existing() -> Result<DoctorSuccess, DoctorFailure> {
        ...
        Err(DoctorFailure {
            message: format!("{}", "No existing kubeconfig has been found. Please add a valid kubeconfig".red()),
            plugin: PLUGIN_NAME.to_string(),
        })
    }
    ...
    fn is_kubeconfig_valid() -> Result<DoctorSuccess, DoctorFailure> {
        if let Err(error) = read_kubeconfig() {
            return Err(DoctorFailure {
                message: format!("{}: {}", "Error while parsing the kubeconfig".red(), error.to_string().yellow()),
                plugin: PLUGIN_NAME.to_string(),
            });
        }
    ...
    fn is_rancher_token_available() -> Result<DoctorSuccess, DoctorFailure> {
        if let Err(error) = get_rancher_token() {
            return Err(DoctorFailure {
                message: format!("{}: {}", "Unable to retrieve Rancher token from credential store".red(), error.to_string().yellow()),
                plugin: PLUGIN_NAME.to_string(),
            })
        }
